...
## Solution
You can achieve this by using the `creator.onConfigureTablePropertyEditor` event. This event allows you to configure property editors for table-like properties (such as choices). By setting `options.allowBatchEdit` to `false`, the bulk edit action is disabled.
